...
async function register(api, body) {
  const issued = await api.post('/api/admin/enrollment-tokens').set('Authorization', 'Bearer admin').send({ tenantId: DEFAULT_TENANT_ID }).expect(201);
  return (await api.post('/api/agents/register').send({ ...body, enrollmentToken: issued.body.token, machineFingerprint: MACHINE_FINGERPRINT }).expect(201)).body;
}
async function envelope(result, identity) {
  ...
  return { schemaVersion: 1, queueItemId: randomUUID(), agentId: identity.agentId, machineFingerprint: MACHINE_FINGERPRINT, enqueuedAt: new Date().toISOString(), contentEncoding: 'gzip', encryption: 'aes-256-gcm', iv: iv.toString('base64'), authTag: cipher.getAuthTag().toString('base64'), ciphertext: ciphertext.toString('base64') };
}
...
test('new registration enqueues only the local baseline bundle and reschedules when due', async (t) => {
  ...
  const app = createApp({ database, adminToken: 'admin', taskSigningSecret: 'test-task-signing-secret', logger, now: () => new Date(clock), baselineRescanIntervalMs: 60000 });
  const api = request(app);
  const identity = await register(api, { hostname: 'baseline-host', platform: 'linux', architecture: 'x64' });
...
test('authenticated fleet websocket receives a push when a decrypted result arrives', async (t) => {
  ...
  const app = createApp({ database, adminToken: 'admin', taskSigningSecret: 'test-task-signing-secret', dashboardSessions: sessions, fleetHub: hub, baselineCollectors: [], logger });
  ...
  const api = request(app);
  const identity = await register(api, { hostname: 'push-host', platform: 'linux', architecture: 'x64' });
